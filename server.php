...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Voornaam is vereist!"); }
  ...
  if ($user) { // if user exists
    if ($user['username'] === $username) {
      array_push($errors, "Naam bestaat al!");
    }
...
if (isset($_POST['login_user'])) {
  ...
  if (empty($username)) {
  	array_push($errors, "Naam is vereist");
  }
  if (empty($password)) {
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Verkeerde combinatie van gebruikersnaam of wachtwoord");
  	}
  }
}
